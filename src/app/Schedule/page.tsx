...
const Schedule = () => {
  ...
  const daySchedule = [
    ...
    { time: '14 h', commitment: 'Atendimento - Doguinho Silva', color: 'bg-blue-500' },
    { time: '15 h', commitment: 'Internamento - Doguinho Silva', color: 'bg-[#9E2C39]' },
    { time: '16 h', commitment: null },
    { time: '17 h', commitment: 'Exame - Doguinho Silva', color: 'bg-[#EA3E51]' },
          ...
          <div className="container mx-auto">
            ...
            <div className="flex items-center justify-between mb-6 w-[450px]">
              <div className="flex space-x-2">
                <button 
                  className={`px-4 py-2 rounded quicksand-semibold ${view === 'day' ? 'bg-blue-600 text-white' : 'bg-gray-300 text-gray-700'}`}
                  onClick={() => setView('day')}
                >
                  Dia
                </button>
                <button 
                  className={`px-4 py-2 rounded quicksand-semibold ${view === 'month' ? 'bg-blue-600 text-white' : 'bg-gray-300 text-gray-700'}`}
                  onClick={() => setView('month')}
                >
                  Mês
                </button>
                <button className="bg-gray-300 text-gray-700 px-4 py-2 rounded quicksand-semibold">Ano</button>
              </div>
                ...
                <table className="w-full table-fixed">
                  <thead>
                    <tr className="bg-gray-50">
                      ...
                      <th className="border-b p-2 text-left text-sm text-green-600 roboto-light tracking-wider">Veterinário</th>
                    </tr>
                  </thead>
                  <tbody>
                    {daySchedule.map((item, index) => (
                      <tr key={index}>
                        <td className="border-b p-2 text-left roboto-light bg-gray-100">{item.time}</td>
                        <td className="border-b p-2"> 
                          {item.commitment ? (
                            <div className={`${item.color} roboto-medium text-white rounded-lg px-2 py-1`}>
                              {item.commitment}
                            </div>
                          ) : (
                            <div> </div> 
									...
									<div className="grid grid-cols-7 gap-0">
										{monthlySchedule.map((item, index) => (
											<div key={index} className="border p-2 h-40"> 
												...
												<div className="mt-2">
													{item.commitments && item.commitments.map((commitment, i) => (
														<div key={i} className={`rounded-lg px-2 py-1 mb-1 ${commitment.color} text-white`}>
															{commitment.name}
														</div>
													))}
												</div>
